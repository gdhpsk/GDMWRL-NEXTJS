...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  await webhook(null, [{
    ...
    description: `[${newLevel.name}](https://youtube.com/watch?v=${newLevel.ytcode}) by ${newLevel.host} (verifier: ${newLevel.verifier}) has just been placed at #${level.position}, above ${between[0]?.name} and below ${between[1]?.name}!`
  }])
